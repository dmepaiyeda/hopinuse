/*!
 * hop: Front-end site library
 * @license MIT
 * Inline scripts may access the api using an async handler:
 *   var hop = hop || [];
 *   hop.push(readyFunction);
 */
...
/**
 * hop: Core site library
 */

var hop = {};
var modules = {};
var primary = [];
var secondary = window.hop || [];
var $ = window.jQuery;
var $win = $(window);
var $doc = $(document);
var isFunction = $.isFunction;
var _ = hop._ = __webpack_require__(5);
var tram = hop.tram = __webpack_require__(1) && $.tram;
var domready = false;
var destroyed = false;
...
/**
 * hop.define - Define a named module
 * @param  {string} name
 * @param  {function} factory
 * @param  {object} options
 * @return {object}
 */
hop.define = function (name, factory, options) {
  if (modules[name]) {
...
/**
 * hop.require - Require a named module
 * @param  {string} name
 * @return {object}
 */
hop.require = function (name) {
  return modules[name];
};

function bindModule(module) {
  // If running in hop app, subscribe to design/preview events
  if (hop.env()) {
...
/**
 * hop.push - Add a ready handler into secondary queue
 * @param {function} ready  Callback to invoke on domready
 */
hop.push = function (ready) {
...
/**
 * hop.env - Get the state of the hop app
 * @param {string} mode [optional]
 * @return {boolean}
 */
hop.env = function (mode) {
...
var touch = hop.env.touch = 'ontouchstart' in window || window.DocumentTouch && document instanceof window.DocumentTouch;
var chrome = hop.env.chrome = /chrome/.test(userAgent) && /Google/.test(navigator.vendor) && parseInt(userAgent.match(/chrome\/(\d+)\./)[1], 10);
var ios = hop.env.ios = /(ipod|iphone|ipad)/.test(userAgent);
hop.env.safari = /safari/.test(userAgent) && !chrome && !ios;
...
/**
 * hop.validClick - validate click target against current touch target
 * @param  {HTMLElement} clickTarget  Element being clicked
 * @return {Boolean}  True if click target is valid (always true on non-touch)
 */
hop.validClick = touch ? function (clickTarget) {
...
/**
 * hop.resize, hop.scroll - throttled event proxies
 */
var resizeEvents = 'resize.hop orientationchange.hop load.hop';
var scrollEvents = 'scroll.hop ' + resizeEvents;
hop.resize = eventProxy($win, resizeEvents);
hop.scroll = eventProxy($win, scrollEvents);
hop.redraw = eventProxy();
...
// hop.location - Wrap window.location in api
hop.location = function (url) {
  window.location = url;
};

if (hop.env()) {
  // Ignore redirects inside a hop design/edit environment
  hop.location = function () {};
}

// hop.ready - Call primary and secondary handlers
hop.ready = function () {
  domready = true;
...
/**
 * hop.load - Add a window load handler that will run even if load event has already happened
 * @param  {function} handler
 */
var deferLoad;
hop.load = function (handler) {
...
// hop.destroy - Trigger a destroy event for all modules
hop.destroy = function (options) {
  options = options || {};
  destroyed = true;
...
/**
 * hop: IX Event triggers for other modules
 */
...
api.types = {
  INTRO: 'hf-ix-intro' + namespace,
  OUTRO: 'hf-ix-outro' + namespace
};
...
/**
 * hop: Brand pages on the subdomain
 */

var hop = __webpack_require__(0);

hop.define('brand', module.exports = function ($) {
  var api = {};
  var doc = document;
  var $html = $('html');
  var $body = $('body');
  var namespace = '.hf---badge';
  ...
  api.ready = function () {
    var shouldBrand = $html.attr('datahop-status');
    var publishedDomain = $html.attr('datahop-domain') || '';
    if (/\.hop\.io$/i.test(publishedDomain) && location.hostname !== publishedDomain) {
      shouldBrand = true;
    }
  ...
  function createBadge() {
    var $brand = $('<a class="hf---badge"></a>').attr('href', 'https://hop.com?utm_campaign=brandjs');

    var $logoArt = $('<img>').attr('src', 'https://d1otoma47x30pg.cloudfront.net/img/hop-badge-icon.60efbf6ec9.svg').css({
      marginRight: '8px',
      width: '16px'
    });

    var $logoText = $('<img>').attr('src', 'https://d1otoma47x30pg.cloudfront.net/img/hop-badge-text.6faa6a38cd.svg');
...
/*!
 * hop._ (aka) Underscore.js 1.6.0 (custom build)
 * _.each
 * _.map
 * _.find
 * _.filter
...
hop.define('forms', module.exports = function ($, _) {
  var api = {};
  ...
  function init() {
    siteId = $('html').attr('datahop-site');

    formUrl = "https://hop.com" + '/api/v1/form/' + siteId;

    // Work around same-protocol IE XDR limitation - without this IE9 and below forms won't submit
    if (retro && formUrl.indexOf("https://hop.com") >= 0) {
      formUrl = formUrl.replace("https://hop.com", "http://formdata.hop.com");
    }
  ...
  function build(i, el) {
    ...
    reset(data);
    var wrap = $el.closest('div.hf-form');
    data.done = wrap.find('> .hf-form-done');
    data.fail = wrap.find('> .hf-form-fail');
    data.fileUploads = wrap.find('.hf-file-upload');
    ...
    data.handler = null;
    data.redirect = $el.attr('hopdata-redirect');
    ...
    // hop forms for hosting accounts
    if (siteId) {
      data.handler = typeof hostedSubmithop === 'function' ? hostedSubmithop : exportedSubmithop;
      return;
    }

    // Alert for disconnected hop forms
    disconnected();
  }
  ...
  function addListeners() {
    ...
    // Handle form submission for hop forms
  ...
  function findFields(form, result) {
    ...
    form.find(':input:not([type="submit"]):not([type="file"])').each(function (i, el) {
      ...
      var name = field.attr('hopdata-name') || field.attr('name') || 'Field ' + (i + 1);
      var value = field.val();
  ...
  function findFileUploads(form) {
    ...
    form.find(':input[type="file"]').each(function (i, el) {
      var field = $(el);
      var name = field.attr('hopdata-name') || field.attr('name') || 'File ' + (i + 1);
  ...
  function exportedSubmithop(data) {
    preventDefault(data);
    afterSubmit(data);
  }
  ...
  function initFileUpload(i, form) {
    ...
    var $defaultWrap = $el.find('> .hf-file-upload-default');
    var $uploadingWrap = $el.find('> .hf-file-upload-uploading');
    var $successWrap = $el.find('> .hf-file-upload-success');
    var $errorWrap = $el.find('> .hf-file-upload-error');
    var $input = $defaultWrap.find('.hf-file-upload-input');
    var $label = $defaultWrap.find('.hf-file-upload-label');
    var $labelChildren = $label.children();
    var $errorMsgEl = $errorWrap.find('.hf-file-upload-error-msg');
    var $fileEl = $successWrap.find('.hf-file-upload-file');
    var $removeEl = $successWrap.find('.hf-file-remove-link');
    var $fileNameEl = $fileEl.find('.hf-file-upload-file-name');

    var sizeErrMsg = $errorMsgEl.attr('hopdata-jinglebell-size-error');
    var typeErrMsg = $errorMsgEl.attr('hopdata-jinglebell-type-error');
    var genericErrMsg = $errorMsgEl.attr('hopdata-jinglebell-generic-error');
 ...
 * 2014-12-16 hop - Removed UMD wrapper
...
hop.define('ix', module.exports = function ($, _) {
  var api = {};
  var designer;
  var $win = $(window);
  var namespace = '.hf-ix';
  ...
  var components = {
    tabs: '.hf-tab-link, .hf-tab-pane',
    dropdown: '.hf-dropdown',
    slider: '.hf-slide',
    navbar: '.hf-nav'
  };
    ...
    _.each(triggers, function (trigger) {
      ...
      if (type === 'click') {
        $el.on('click' + namespace, function (evt) {
          ...
          if (!hop.validClick(evt.currentTarget)) {
            return;
          }
  ...
  function run(trigger, $el, opts, replay) {
    ...
    if (!replay) {
      ...
      if (selector) {
        ...
        if (inApp) {
          $el.attr('hophopdata-ix-affect', 1);
        }
      }

      // Apply empty fix for certain Chrome versions
      if (emptyFix) {
        $el.addClass('hf-ix-emptyfix');
      }
...
/**
 * hop: Auto-select links to current page or section
 */

var hop = __webpack_require__(0);

hop.define('links', module.exports = function ($, _) {
  var api = {};
  var $win = $(window);
  var designer;
  var inApp = hop.env();
  ...
  function init() {
    designer = inApp && hop.env('design');
    slug = hop.env('slug') || location.pathname || '';
...
/**
 * hop: Navbar component
 */

var hop = __webpack_require__(0);
var IXEvents = __webpack_require__(11);

hop.define('navbar', module.exports = function ($, _) {
  var api = {};
  var tram = $.tram;
  var $win = $(window);
  var $doc = $(document);
  ...
  var inEditor;
  var inApp = hop.env();
  var overlay = '<div class="hf-nav-overlay" datahop-ignore />';
  var namespace = '.hf-nav';
  var buttonOpen = 'hf--open';
  var menuOpen = 'hf--nav-menu-open';
  var linkOpen = 'hf--nav-link-open';
  ...
  function removeListeners() {
    hop.resize.off(resizeAll);
  }

  function addListeners() {
    hop.resize.on(resizeAll);
  }
  ...
  function build(i, el) {
    ...
    data.menu = $el.find('.hf-nav-menu');
    data.links = data.menu.find('.hf-nav-link');
    data.dropdowns = data.menu.find('.hf-dropdown');
    data.button = $el.find('.hf-nav-button');
    data.container = $el.find('.hf-container');
  ...
  function configure(data) {
    ...
    var animation = config.animation = data.el.attr('hopdata-animation') || 'default';
    ...
    config.easing = data.el.attr('hopdata-easing') || 'ease';
    config.easing2 = data.el.attr('hopdata-easing2') || 'ease';

    var duration = data.el.attr('hopdata-duration');
    config.duration = duration != null ? Number(duration) : 400;

    config.docHeight = data.el.attr('hopdata-doc-height');
  ...
  function outside(data) {
    ...
    return function (evt) {
      ...
      if (inEditor && $target.closest('.hf-editor-bem-EditorOverlay').length) {
        return;
      }
  ...
  var outsideDebounced = _.debounce(function (data, $target) {
    ...
    var menu = $target.closest('.hf-nav-menu');
  ...
  function close(data, immediate) {
    ...
    function complete() {
      ...
      data.el.triggerHandler('hf-close');
    }
  }
...
hop.define('scroll', module.exports = function ($) {
  var $doc = $(document);
  var win = window;
  ...
  function findEl(hash, e) {
    ...
    !(hop.env.chrome && loc.protocol === 'file:')) {
    ...
    var rootTag = hop.env('editor') ? '.hf-editor-body' : 'body';
    var header = $('header, ' + rootTag + ' > .header, ' + rootTag + ' > .hf-nav:not([hopdata-no-scroll])');
  ...
  function scroll(el, offset) {
    ...
    $('body').add(el).each(function () {
      var time = parseFloat($(this).attr('hopdata-scroll-time'), 10);
...
hop.define('touch', module.exports = function ($) {
  var api = {};
